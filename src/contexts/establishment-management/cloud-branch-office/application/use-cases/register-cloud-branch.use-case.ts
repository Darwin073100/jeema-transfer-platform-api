...
export class RegisterCloudBranchUseCase {
  ...
  async execute(dto: RegisterCloudBranchDTO) {
    try {
      ...
      return await this.transactionDB.runInTransaction(async () => {
        ...
        const establishment =
          await this.cloudEstablishmentRepository.existByEnrollmentKey(
            dto.enrollmentKey,
          );
        if (!establishment) {
        ...
        const branchOffice = CloudBranchOfficeEntity.create(
          dto.branchOfficeName,
          establishment.cloudEstablishmentId,
          dto.localBranchOfficeId,
        );
        // Guardar la sucursal
        const branchOfficeResult =
          await this.cloudBranchOfficeRepository.save(branchOffice);
